...
def names_to_apa(a_string):
    # print(a_string)
    new_string = a_string.replace(" and ","")
    print(new_string)
    # new_string = new_string[:11]+new_string[12:]
    # print(new_string)
    a_list = new_string.split(",")
    # a_list.remove()
    ret_list = []
    print(a_list)
    for each_name in range(0, len(a_list)):
        b_list = a_list[each_name].split()

        retb_sting = b_list[1]+", "+b_list[0][0]+"., "
        ret_list.append(retb_sting)
    print(ret_list)
    reta_string = "".join(ret_list)
    # print(reta_string)
    retc_string = reta_string.rstrip(", ")
    return retc_string
...
# print(names_to_apa("Edith Clark, Margaret Hamilton, and Isaac Asimov"))
#Clark, E., Hamilton, M., Asimov, I.
