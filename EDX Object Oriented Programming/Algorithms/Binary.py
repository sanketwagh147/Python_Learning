...
def binary_search_year(a_list, target):
    # print(a_list)
    # print(a_list)
    a_list.sort()
    if len(a_list) == 0:
        ...
        # print(a_list[midpoint].year)
        if a_list[midpoint].year == target:
            return True
        else:
            if a_list[midpoint].year < target:
                return binary_search_year(a_list[midpoint + 1:], target)
            else:
                return binary_search_year(a_list[:midpoint], target)
