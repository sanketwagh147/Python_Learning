...
def pivot_library(a_list):
    ...
    for each_tuple in a_list:
        dict_value = []
        # print(each_tuple)
        dict_value.append(each_tuple[0])
        dict_value.append(each_tuple[1])
        r_dictionary[each_tuple[2]]=tuple(dict_value)
        r_list.append(each_tuple[2]+": "+each_tuple[0]+" by "+each_tuple[1]+"\n")
    # print(r_dictionary)
    return r_list

def write_book_info(a_string, a_list):
    r_dict = pivot_library(a_list)
    print(r_dict)
    a_file = open(a_string, "w")
    a_file.writelines(r_dict)
