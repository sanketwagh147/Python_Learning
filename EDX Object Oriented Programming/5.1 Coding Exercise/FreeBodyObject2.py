...
def find_net_force(a_list):
    total_horizontal = 0
    total_vertical= 0
    for each_force in a_list:
        total_horizontal += each_force.get_horizontal()
        total_vertical += each_force.get_vertical()
    net_force = round((total_horizontal**2 + total_vertical**2)** 0.5, 1)
    final_angle =  atan2(total_vertical, total_horizontal)
    final_angle_in_degrees = round(degrees(final_angle), 1)
    # print(net_force)
    # print(angle)
    return Force(net_force, final_angle_in_degrees)
