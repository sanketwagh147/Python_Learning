...
def total_stats(a_list_of_dictionaries):
    individual_dict={}
    for each_pokemon in a_list_of_dictionaries:
        # print(each_pokemon)
        each_stat_total = 0
        each_stat_total = each_pokemon["hp"]+each_pokemon["attack"]+each_pokemon["defense"]+each_pokemon["special attack"]+each_pokemon["special defense"]+each_pokemon["speed"]
        individual_dict[each_pokemon["name"]] = each_stat_total
        # print(each_stat_total)
...
def total_stats(list_of_pokemon):
    ...
    #We're going to want to return a dictionary at the end.
    #So, let's go ahead and create that dictionary as an empty
    #dictionary so we can add stuff to it as we go:
    results = {}

    #Now, we want to iterate through each Pokemon in the list:
    for mon in list_of_pokemon:

        #For each one, we want to get its name...
        name = mon["name"]

        #...and add up its total stats. Here, I'm doing that
        #by creating total_stats and setting it equal to HP,
        #then adding other values -- this is just to keep the
        #lines short. You could also just have one long line
        #adding all six up:
        total_stats = mon["hp"]
        ...
        #Finally, we create a new key in the dictionary with the
        #Pokemon's name, and set its value to the total stats:
        results[name] = total_stats

    #At the end, we return the dictionary we built:
    return results
