...
def check_winner(tuples):
        # tuples[x][y] is the symbol we are looking at
    x_min = x
    x_max = x

    # now we check to the left how many symbols we find that are the same
    while 0 < x_min:
        if tuples[x_min-1][y] == tuples[x][y]:
            x_min -= 1
        else:
            break

    # now we look how far to the right we can go
    # here x_bound is the maximum value x can be which in your test cases would be 6
    while x_bound > x_max:
        if tuples[x_max+1][y] == tuples[x][y]:
            x_max += 1
        else:
            break

    # now we know the indexes of the left and rightmost symbol that is the same as tuples[x][y]
    # and we can just calculate how many there are in between
    adjacent = x_max - x_min + 1
