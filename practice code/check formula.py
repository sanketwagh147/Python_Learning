...
def check_formula(a_string):
    allowed_operators = ["+", "-", "*", "/", "="]
    not_allowed_operators = [" ", ".", ",",]
    if a_string[0] in allowed_operators:
        # print("1")
        return False
    if "=" not in a_string:
        # print("2")
        return False
    even_item = a_string[::2]
    # print(even_item)
    even_item_list = list(map(str, even_item))
    # print(even_item_list)
    for each_item in even_item_list:
        # print("3")
        if each_item in allowed_operators:
            # print("4")
            return False
    if a_string[::2] in allowed_operators:
        # print("5")
        return False
    for each_char in a_string:
        # print("6")
        # print(each_char)
        if ord(each_char) >= 64 or each_char in not_allowed_operators:
            # print("character found")
            # print("7")
            return False
    if a_string[-1].isdigit() == False:
        # print("8")
        return False
...
print(check_formula("9+1/3+2=7*5*4"))
