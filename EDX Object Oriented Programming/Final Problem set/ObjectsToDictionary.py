...
#Add your code here!
def to_dictionaries(a_list_Nation):
    ret2_dict={}
    for each in a_list_Nation:
        ret_dict ={}
        name = each.short_name
        a_dict={}
        a_dict["long_name"]=each.long_name
        a_dict["iso_code"]=each.iso_code
        a_dict["iso_short"]=each.iso_short
        a_dict["iso_long"]=each.iso_long
        a_dict["capital"]=each.capital
        # print(a_dict)
        # print(type(a_dict))
        s_name = each.short_name
        ret_dict[str(s_name)] = a_dict
        ret2_dict.update(ret_dict)
        print(ret_dict)
    return ret2_dict
